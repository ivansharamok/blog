...
# compiles configuration properties for dynamic config used in zk 3.5+ versions.
function print_servers_dynamic() {
...
# compiles server config properties for static configuration file (pre zk 3.5 versions).
function print_servers() {
    for (( i=0; i<$ZK_REPLICAS; i++ ))
    do
      echo "server.$i=$NAME-$((i)).$DOMAIN:$ZK_SERVER_PORT:$ZK_ELECTION_PORT"
    done
}
...
function create_config() {
    ...
    echo "clientPort=$ZK_CLIENT_PORT" >> $ZK_STATIC_CONFIG
    print_servers >> $ZK_STATIC_CONFIG
    # starting from version 3.5 dynamic config can be used to configure dynamic properties like servers, roles, clientPort.
    # uncomment line below for 3.5+ zk version and move dynamic properties into dynamic config.
    ...
    # uncomment if dynamic config is used in zk version 3.5+
    #print_servers_dynamic >> $ZK_DYNAMIC_CONFIG
...
function create_log4j_props() {
    ...
    # example to set DEBUG level logging with rolling file
    #echo "zookeeper.root.logger=DEBUG, CONSOLE, ROLLINGFILE" >> $LOGGER_PROPS
    echo "zookeeper.root.logger=INFO, CONSOLE" >> $LOGGER_PROPS
